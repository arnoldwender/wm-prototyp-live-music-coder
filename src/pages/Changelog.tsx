...
export default function Changelog() {
  ...
  return (
    <main className="min-h-screen" style={{ backgroundColor: 'var(--color-bg)', color: 'var(--color-text)' }}>
      <a href="#changelog-list" className="sr-only focus:not-sr-only" style={{ position: 'absolute', top: 'var(--space-2)', left: 'var(--space-2)', zIndex: 100, padding: 'var(--space-2) var(--space-4)', backgroundColor: 'var(--color-primary)', color: 'var(--color-text)', borderRadius: 'var(--radius-sm)' }}>
        Skip to changelog
      </a>
      ...
      {/* Changelog timeline */}
      <section id="changelog-list" aria-label={t('changelog.listAriaLabel')} style={{ maxWidth: '900px', margin: '0 auto', padding: '0 var(--space-6) var(--space-8)' }}>
        {sorted.length === 0 ? (
            ...
            {sorted.map((entry, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 flex-wrap" style={{ marginBottom: 'var(--space-2)' }}>
                  ...
                  {entry.version && (
                    <span style={{ fontSize: 'var(--font-size-xs)', fontWeight: 600, color: 'var(--color-primary)', fontFamily: 'var(--font-family-mono)' }}>
                      v{entry.version}
                    </span>
                  )}
