...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    body: [
      ...
      'The first diagnosis was wrong and measurement corrected it: moving the app',
      'into Applications changed nothing. What blocked it was a leftover updater',
      'folder owned by another user, which the message now names, with the command',
      'to remove it. The app being on a read-only volume is a second, independent',
      'cause and is still covered. The message is matched on the part the system',
      'writes in English, since the rest arrives in the system language.',
    ].join('\n'),
  },
  {
    version: '1.3.0',
    ...
    category: 'feature',
    body: [
      'createParams() returned a plain number snapshotted at evaluation time, so',
      'setParam() changed a value the running pattern never read again. It now',
      'returns a Strudel ref, which is queried every cycle — the pattern picks up',
      'the new value without being re-evaluated.',
      '',
      'The documented "or via MIDI CC mapping" half did not exist: MIDI Learn',
      'stored a parameter-to-CC map that nothing read, so finishing the Learn flow',
      'left the knob controlling nothing. Learned mappings are now applied from the',
      'same CC stream the synth panel uses, and you choose which parameter to map',
      'instead of everything landing on lpf.',
      '',
      'The node graph no longer shows an empty state for Strudel. A pattern is one',
      'expression but not a flat one: $: voices run in parallel, a chain of calls is',
      'ordered, and stack() mixes independent branches. All four engines are now',
      'laid out in columns by signal depth, so an edge always points forward.',
      '',
      'The Strudel and MIDI graphs are views — their code cannot be reassembled',
      'from node snippets, and the canvas says so rather than swallowing drags.',
      '',
      'A failed update no longer looks like a working one. On macOS the "restart',
      'now?" prompt appears before the new build has been validated; if validation',
      'then fails, the restart button used to do nothing at all and retry silently',
      'on every quit. Terminal failures now explain themselves and offer the',
      'download page.',
    ].join('\n'),
  },
  {
